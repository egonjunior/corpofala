...
import FunnelWhatsApp from "@/components/funnel/FunnelWhatsApp";
...
const FunilLigacao = () => {
  const [screen, setScreen] = useState<"unlock" | "call" | "whatsapp" | "instagram" | "lp">("unlock");
      ...
      <AnimatePresence mode="wait">
        ...
        {screen === "whatsapp" && (
          <motion.div key="whatsapp" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }} className="w-full h-full">
            <FunnelWhatsApp onNext={() => setScreen("instagram")} />
          </motion.div>
        )}
        {screen === "instagram" && (
