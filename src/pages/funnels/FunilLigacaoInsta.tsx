...
import FunnelInstagram from "@/components/funnel/FunnelInstagram";

// Let's create the full flow in one file similar to FunilLigacao.tsx to avoid circular dependencies for now
...
const FunilLigacaoInsta = () => {
    const [screen, setScreen] = useState<"unlock" | "call" | "instagram" | "lp">("unlock");
            ...
            <AnimatePresence mode="wait">
                ...
                {screen === "call" && (
                    <motion.div key="call" initial={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.5 }} className="w-full h-full">
                        <FunnelCall onNext={() => setScreen("instagram")} />
                    </motion.div>
                )}
                {screen === "instagram" && (
                    <motion.div key="instagram" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }} className="w-full h-full">
                        <FunnelInstagram onBioLink={() => setScreen("lp")} />
                    </motion.div>
                )}
                {screen === "lp" && (
...
export default FunilLigacaoInsta;
